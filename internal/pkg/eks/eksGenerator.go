...
func ProtoGenerate(model marketplace.Install_Extensions_Eks) (string, error) {
	...
	tags := AWSTags{}
	config := EKSConfig{
		ServiceArn:              model.EKSServiceArn,
		ClusterName:             model.Clustername,
		ClusterRegion:           model.EKSClusterRegion,
		ClusterVersion:          model.EKSClusterVersion,
		ClusterInstanceType:     model.Nodegroups[0].Instancetype,
		ManagedNodeGroups:       nodeGroups,
		ClusterAMI:              model.EKSClusterAMI,
		InstanceRoleArn:         model.EKSInstanceRoleArn,
		KubeProxyVersion:        model.EKSKubeProxyVersion,
		CoreDNSVersion:          model.EKSCoreDNSVersion,
		EBSCSIVersion:           model.EKSEBSCSIVersion,
		PublicSubnetA:           model.EKSPublicSubnetA,
		PublicSubnetB:           model.EKSPublicSubnetB,
		PrivateSubnetA:          model.EKSPrivateSubnetA,
		PrivateSubnetB:          model.EKSPrivateSubnetB,
		SecurityGroup:           model.EKSSecurityGroup,
		SharedNodeSecurityGroup: model.EKSSharedNodeSecurityGroup,
		Tags:                    tags,
	}
	tmpl, err := template.New("eks-config").Parse(templates.Eksctl)
	if err != nil {
